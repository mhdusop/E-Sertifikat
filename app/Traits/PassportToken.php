...
trait PassportToken
{
    ...
    {
        $accessToken = new AccessToken($user->uuid);
     ...
     */
    protected function getBearerTokenByUser(User $user, $clientId, $clientSecret, $output = true, $need_validate_client = true)
    {
        if ($need_validate_client) {
            if (is_null(\DB::table('oauth_clients')->where('id', $clientId)->where('secret', $clientSecret)->where('password_client', 1)->first())) {
                throw OAuthServerException::serverError('invalid client_id');
            }
        }
